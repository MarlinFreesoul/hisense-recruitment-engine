...
function getDraftSecret() {
	return "85140734-1929-420f-b23d-96b7a8ca848e";
}
/**
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "6d1fed13-76ac-407a-b3ca-386b0d8f2615";
	}
};
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "6d1fed13-76ac-407a-b3ca-386b0d8f2615") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "6d1fed13-76ac-407a-b3ca-386b0d8f2615",
		rootBoundaryId
	});
}
/**
...
var CREATE_SQL = `CREATE TABLE IF NOT EXISTS audit_logs (
...
async function POST(request) {
	...
	await env.DB.prepare(CREATE_SQL).run();
	await env.DB.prepare("INSERT INTO audit_logs (action, entity_type, entity_id, actor, detail, created_at) VALUES (?, ?, ?, ?, ?, ?)").bind(body.action, body.entityType, body.entityId, "demo-recruiter", body.detail ?? null, (/* @__PURE__ */ new Date()).toISOString()).run();
	return Response.json({
		ok: true,
		mode: "persistent"
	});
...
var routes = [{
	...
	ids: {
		"route": "route:/api/audit",
		"page": null,
		"routeHandler": "route-handler:/api/audit",
		"rootBoundary": "root-boundary:/",
		"layouts": ["layout:/"],
		"templates": [],
		"slots": {}
...
var __publicFiles = new Set([
	"/data/hisense_boss_jobs_structured.md",
	"/favicon.svg",
	"/file.svg",
	"/globe.svg",
	"/interview_form.html",
