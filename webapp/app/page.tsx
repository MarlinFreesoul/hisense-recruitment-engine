...
import { fetchJobs, fetchResults, fetchInterviewQuestions, submitInterview } from "../lib/api";
import type { ApiJob, ApiResult, ApiQuestion } from "../lib/api";
...
function Screening({ candidate, step, setStep, onSchedule }: { candidate: Candidate; step: number; setStep: (n: number) => void; onSchedule: () => void }) {
  ...
  useEffect(() => {
    setStep(0);
    setAnswers({});
    fetchInterviewQuestions(candidate.name).then((d) => {
